...
def _is_reference_start(line: str) -> bool:
    return bool(
        re.match(r"^(\[\d+\]|\d+\.|[A-Z][A-Za-z'`-]+,\s+[A-Z])", line)
...
def _parse_reference(entry: str) -> Reference:
    ...
    year_match = re.search(r"(?:19|20)\d{2}[a-z]?", raw)
    doi_match = _DOI_PATTERN.search(raw)
    url_match = _URL_PATTERN.search(raw)

    authors_part = raw[: year_match.start()].strip(" .,") if year_match else raw.split(".", maxsplit=1)[0]
    authors = [part.strip() for part in re.split(r"\band\b|,|;", authors_part) if part.strip()]
    ...
    if title_match:
        ...
        title = after_year.split(".", maxsplit=1)[0].strip() if after_year else None

    venue = None
    if title and title in raw:
        after_title = raw.split(title, maxsplit=1)[-1].strip(" .,")
        if after_title:
            venue = after_title.split(".", maxsplit=1)[0].strip()
